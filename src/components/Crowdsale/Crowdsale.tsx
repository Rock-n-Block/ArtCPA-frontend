...
const fromOptionTypeToTokenType = (
  value: OptionType,
  listOfTokens: TSingleTokenWithIcon[],
) => {
  const selectedToken = listOfTokens.find(
    (token) => token.address === value.value,
  );
  if (selectedToken) {
...
export const Crowdsale: VFC<CrowdsaleProps> = ({ className }) => {
  ...
  const availableTokens = useMemo(
    () => tokens.map<TSingleTokenWithIcon>((t) => ({
      ...t,
      icon: tokensIconsMap[t.symbol],
    })),
    [tokens],
  );

  const [select, setSelect] = useState(
    fromTokenTypeToOptionType(availableTokens[0]),
  );
  ...
  const selectedFullInfo = useMemo(
    () => fromOptionTypeToTokenType(select, availableTokens),
    [select, availableTokens],
  );

  const { requestCurrentStage, requestStageTimeLeft } = useContractMethods();

  const { stage, stageTimeLeft, stageTokenPrice, stageLimits } =
    useShallowSelector(crowdSaleSelector.geTCrowdSaleState);
  ...
  const mainTokenBalance = useMemo(() => {
    const mainTokenInfo = userTokens.find(
      (token) => token.identifier === MainToken.address,
    );
    if (mainTokenInfo) {
  ...
  const payableTokenBalance = useMemo(() => {
    const payableTokenInfo = userTokens.find(
      (token) => token.identifier === select.value,
    );
    if (payableTokenInfo) {
      return decimalNumber({
        value: new BigNumber(payableTokenInfo.balance),
        decimals: new BigNumber(payableTokenInfo.decimals),
      });
    }
  ...
  const payableTokenToMainToken = useMemo(() => {
    if (selectedFullInfo?.price) {
      return new BigNumber(selectedFullInfo.price)
        .div(stageTokenPrice)
        .toNumber();
    }
    return 1;
  ...
  const minimumLimit = useMemo(() => {
    if (stageLimits.minimum && selectedFullInfo?.price && stage?.leftTokens) {
      ...
      const leftInPayableToken = leftWithoutDecimals.multipliedBy(
        mainTokenToPayableToken,
      );
      const rawLimit = stageLimits.minimum
        .div(selectedFullInfo.price)
        .toNumber();
      return leftInPayableToken.isLessThan(rawLimit)
        ? leftInPayableToken.toNumber() - leftInPayableToken.toNumber() / 2
        : rawLimit;
    }
    return 0;
  ...
  const maximumLimit = useMemo(() => {
    if (stageLimits.maximum && selectedFullInfo?.price && stage?.leftTokens) {
      ...
      const rawLimit = stageLimits.maximum
        .div(selectedFullInfo.price)
        .toNumber();
      return leftInPayableToken.isGreaterThan(rawLimit)
        ? rawLimit
        : leftInPayableToken.toNumber();
    }
    return 0;
  }, [mainTokenToPayableToken, selectedFullInfo, stage, stageLimits.maximum]);

  const handleChangeSendInput = useCallback(
    (event) => {
  ...
  const sendErrors = useMemo(() => {
    ...
    if (!Object.is(parseFloat(sendInput), NaN)) {
      ...
      if (
        new BigNumber(parseFloat(sendInput)).isLessThan(minimumLimit.toFixed(5))
      ) {
  ...
  const handleChangeReceiveInput = useCallback(
    (event) => {
      if (validateOnlyNumbers(event.target.value)) {
  ...
  const receiveErrors = useMemo(() => {
    ...
    if (!Object.is(parseFloat(receiveInput), NaN)) {
      if (
        new BigNumber(
          parseFloat(receiveInput) * mainTokenToPayableToken,
        ).isGreaterThan(maximumLimit)
      ) {
      ...
      if (
        new BigNumber(
          parseFloat(receiveInput) * mainTokenToPayableToken,
        ).isLessThan(minimumLimit.toFixed(5))
      ) {
      ...
      dispatch(
        buyToken({
          data: {
            ...
            amountToPay: decimalNumber({
              value: new BigNumber(sendInput),
              decimals: new BigNumber(selectedFullInfo.decimals),
              format: 'with',
            }).toFixed(0),
          ...
          onSignatureGet: (signature) => {
            sendMethod({
              args: [
                new BytesValue(signature.msg),
                new BytesValue(signature.signature),
              ],
              method: 'pay',
  ...
  const allowToCrowd = useMemo(
    () => parseFloat(receiveInput) < 0 ||
      isFetching ||
      Object.is(parseFloat(receiveInput), NaN) ||
      parseFloat(sendInput) > maximumLimit ||
      sendErrors.length !== 0 ||
      receiveErrors.length !== 0,
    [
      isFetching,
      maximumLimit,
      receiveErrors.length,
      receiveInput,
      sendErrors.length,
      sendInput,
    ],
  ...
  return (
    <WrapContainer name={HomePageAnchors.BUY} className={styles.smainWrapper}>
      <H1 align="center" className={styles.mainTitle} weight="bold">
        Crowdsale
      </H1>
      <div className={cn(styles.crowdsale, className)}>
        <H2 align="center" className={styles.title} weight="semiBold">
          {stage
            ? `${stage.stageNumber.toNumber() + stageAddition} ${
              crutch ? 'phase will start in' : 'phase is live!'
            }`
            : `${MainToken.symbol} crowdsale is over`}
        </H2>
        {stage && (
          <>
            <Countdown
            ...
            <ProgressBar
              text={formatNumber(
                decimalNumber({ value: stage.totalTokens })
                  .minus(tokensBought)
                  .toNumber(),
              ).toString()}
            ...
            {!isStageSoldOut ? (
              <>
                <div className={styles.purchaseAmount}>
                  <Text noWrap={false} className={styles.purschAmount}>
                    Min purchase amount:{' '}
                    <Text>
                      {new BigNumber(minimumLimit).decimalPlaces(5).toString()}{' '}
                      {select.label}
                    </Text>
                  </Text>
                  <Text
                    noWrap={false}
                    align="right"
                    className={styles.purschAmount}
                  >
                    Max purchase amount:{' '}
                    <Text align="right">
                      {new BigNumber(maximumLimit).decimalPlaces(5).toString()}{' '}
                      {select.label}
                    </Text>
                  </Text>
                </div>
                ...
                <div className={styles.textUnderUnput}>
                  <Text color="secondary" noWrap={false}>
                    Your {select.value} balance{' '}
                    {payableTokenBalance.decimalPlaces(5).toString()}{' '}
                    {select.label}
                  </Text>
                  <Text color="secondary">
                    1 {select.label} ={' '}
                    {new BigNumber(selectedFullInfo?.price || 0)
                      .decimalPlaces(5)
                      .toString()}
                    $
                  </Text>
                ...
                <div className={styles.wrapInputWithSelect}>
                  ...
                  <Button
                    ...
                    startAdorment={<Coin width="30" height="30" />}
                    className={styles.CPAbtn}
                  >
                    {MainToken.symbol}
                  </Button>
                </div>
                <div className={styles.textUnderUnput}>
                  <Text color="secondary" noWrap={false}>
                    Your {MainToken.address} balance{' '}
                    {mainTokenBalance.decimalPlaces(5).toString()}{' '}
                    {MainToken.symbol}
                  </Text>
                  <Text color="secondary">
                    1 {MainToken.symbol} ={' '}
                    {stageTokenPrice.decimalPlaces(5).toString()}$
                  </Text>
                </div>
                {userNfts.length > 0 ? (
                  <div>
                    <Text size="s" color="secondary" align="center">
                      You&apos;ll get +10% {MainToken.symbol} because you have
                      NFTs from {userNfts.join(',')}
                    </Text>
                    <Text size="s" color="secondary" align="center">
                      {new BigNumber(+receiveInput * 1.1)
                        .decimalPlaces(5)
                        .toNumber()}{' '}
                      {MainToken.symbol}
                    </Text>
                  ...
                  <div>
                    <Text size="s" color="secondary" align="center">
                      You&apos;ll get +10% {MainToken.symbol} if you have NFTs
                      from {nftWithDiscount.join(',')}
                    </Text>
                  </div>
                )}
                <div className={styles.buyInfo}>
                  <Text noWrap={false} align="center">
                    You buy ArtCPAclub Tokens by sending {select.value} to the
                    contract
                  </Text>
                </div>
                <Button
                  disabled={allowToCrowd || !address || crutch}
                  variant="filled"
                  className={styles.buyButton}
                  size="md"
                  onClick={onBuyClickHandler}
                >
                  BUY {MainToken.symbol}
                </Button>
              </>
            ) : (
              <Text noWrap={false} align="center">
                {stage.stageNumber.toNumber() + stageAddition} phase is sold
                out!
              </Text>
            )}
          </>
